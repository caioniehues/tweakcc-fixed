...
const patchDescriptionFunction = (oldFile: string): string | null => {
  // Pattern matches old versioned ("Opus 4.6 in plan mode, else Sonnet 4.6") and
  // new generic ("Opus in plan mode, else Sonnet") opusplan descriptions; CC >=
  // 2.1.113 dropped the model version numbers. Group 3 captures the description
  // text so the opusplan[1m] case can reuse it.
  const pattern =
